...
if (isset($_GET['search']) AND isset($_GET['status'])) {
	...
	if (!empty($_GET['search']) AND !empty($_GET['status'])) {
	   $query_list = "SELECT * FROM orders_pulsa WHERE phone LIKE '%$search%' OR oid LIKE '%$search%' AND status LIKE '%$status%' AND user = '$sess_username' ORDER BY id DESC"; // edit
	} else if (empty($_GET['search'])) {
	   $query_list = "SELECT * FROM orders_pulsa WHERE status LIKE '%$status%' AND user = '$sess_username' ORDER BY id DESC"; // edit 
	} else if (empty($_GET['status'])) {
	   $query_list = "SELECT * FROM orders_pulsa WHERE phone LIKE '%$search%' OR oid LIKE '%$search%' AND user = '$sess_username' ORDER BY id DESC"; // edit 
	} else {
	    $query_list = "SELECT * FROM orders_pulsa WHERE user = '$sess_username' ORDER BY id DESC"; // edit
	}
} else {
    $query_list = "SELECT * FROM orders_pulsa WHERE user = '$sess_username' ORDER BY id DESC"; // edit
}
...
// end paging config
												...
												?>
													<tr class="odd gradeX">
														<td><a href="<?php echo $cfg_baseurl; ?>orders/detail/pulsa?oid=<?php echo $data_order['oid']; ?>" class="btn btn-primary btn-xs dim" title="Detail"><i class="fa fa-eye"></i></a></td>	
														<td><?php echo $data_order['oid']; ?></td>
														<td><?php echo $data_order['service']; ?></td>
														<td><?php echo $data_order['phone']; ?></td>
														<td><?php echo $data_order['nometer']; ?></td>
