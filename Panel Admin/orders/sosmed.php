...
if (isset($_SESSION['user'])) {
	...
	if (isset($_POST['order'])) {
		...
		$oid = S1.-random_number(3).random_number(3);
		...
		if (empty($post_service) || empty($post_link) || empty($post_quantity)) {
		...
		} else if (mysqli_num_rows($check_orders) == 2) {
		    $msg_type = "error";
            ...
            if ($provider == "SPETRMEDIA" AND $json_result->error == TRUE) {
                $msg_type = "error";
                $msg_content = "<b>Gagal:</b> ".$json_result->error;
            } else {
                if ($provider == "SPETRMEDIA") {
                    $poid = $json_result->id;
