	...
	if (mysqli_num_rows($check_user) == 0) {
		...
		if (isset($_GET['sid'])) {
			...
			if (mysqli_num_rows($checkdb_service) == 0) {
				header("Location: ".$cfg_baseurl."admin/servicess.php");
			} else {
				if (isset($_POST['edit'])) {
					...
					if (empty($post_service) || empty($post_note) || empty($post_min) || empty($post_max) || empty($post_price) || empty($post_pid) || empty($post_provider)) {
						...
						$update_service = mysqli_query($db, "UPDATE servicess SET category = '$post_cat', service = '$post_service', note = '$post_note', min = '$post_min', max = '$post_max', price = '$post_price', status = '$post_status', pid = '$post_pid', provider = '$post_provider' WHERE sid = '$post_sid'");
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default animated fadeInDown">
                        <div class="panel-heading">
                           <h3 class="panel-title"><i class="fa fa-edit"></i> Ubah Layanan</h3>
                                </div>
                		<div class="panel-body">
										...
										<form class="form-horizontal" role="form" method="POST">
											...
											<div class="form-group">
												...
												<button type="submit" class="btn btn-primary" name="edit"><i class="fa fa-edit"></i> Ubah</button>
											<a href="<?php echo $cfg_baseurl; ?>admin/servicess" class="btn btn-warning"><i class="fa fa-refresh"></i> Kembali</a>
											    </div>
										    </div>
										</form>
									</div>
