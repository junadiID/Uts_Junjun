	...
	if (mysqli_num_rows($check_user) == 0) {
		...
		if (isset($_POST['add'])) {
			...
			if (empty($post_sid) || empty($post_service) || empty($post_note) || empty($post_min) || empty($post_max) || empty($post_price) || empty($post_pid) || empty($post_provider)) {
				...
				$insert_service = mysqli_query($db, "INSERT INTO servicess (sid, category, service, note, min, max, price, status, pid, provider) VALUES ('$post_sid', '$post_cat', '$post_service', '$post_note', '$post_min', '$post_max', '$post_price', 'Active', '$post_pid', '$post_provider')");
