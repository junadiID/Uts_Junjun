...
$check_order = mysqli_query($db, "SELECT * FROM orderss WHERE status IN ('Pending','Processing') AND provider = 'SP2'");
...
if (mysqli_num_rows($check_order) == 0) {
  ...
  if ($o_provider == "MANUAL") {
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, 'https://spetr-media.com/api/sosmed2/status');
    ...
    $update_order = mysqli_query($db, "UPDATE orderss SET status = '$un_status', remains = '$u_remains', start_count = '$u_start' WHERE poid = '$o_poid'");
