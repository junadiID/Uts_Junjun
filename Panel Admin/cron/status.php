...
$check_order = mysqli_query($db, "SELECT * FROM orders WHERE status IN ('Pending','Processing') AND provider = 'SP'");
...
if (mysqli_num_rows($check_order) == 0) {
  ...
  if ($o_provider == "MANUAL") {
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, 'https://spetr-media.com/api/sosmed/status');
