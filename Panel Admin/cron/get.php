...
$api_postdata = "key=DSwOESlJpxJq8lR9zcdp&action=service";
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "https://spetr-media.com/api/sosmed/service");
...
// setting price 
 $check_services = mysqli_query($db, "SELECT * FROM services WHERE pid = '$sid' AND provider='SPETRMEDIA'");
...
$insert=mysqli_query($db, "INSERT INTO services (sid,category,service,note, min, max, price, status, pid, provider) VALUES ('$sid','$category','$service','$note','$min_order','$max_order','$rate_asli','Active','$sid','SPETRMEDIA')");//Memasukan Kepada Database (OPTIONAL)
if($insert == TRUE){
