...
$check_order = mysqli_query($db, "SELECT * FROM orderss WHERE status IN ('Error','Partial') AND refund = '0'");
...
if (mysqli_num_rows($check_order) == 0) {
	...
	while($data_order = mysqli_fetch_assoc($check_order)) {
		...
		$u_remains = $data_order['remains'];
    		...
    		$update_order = mysqli_query($db, "UPDATE orderss SET refund = '1'  WHERE oid = '$o_oid'");
